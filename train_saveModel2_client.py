...
def do_inference(hostport, work_dir, concurrency, num_tests):
  ...
  for index in range(int(num_tests/concurrency)):
    ...
    results2 = tf.contrib.util.make_ndarray(response.outputs['predict_prob'])
    pred_res[0].append(list(results1))
    pred_res[1].append(list(results2))
    ...
    '''
  pred_res1 = np.array(pred_res[0]).reshape((-1,1))
  pred_res2 = np.array(pred_res[1]).reshape((-1,1))
  real_res  = np.array(real_res).reshape((-1,1))
  #print ('pred_res1.shape:', pred_res1.shape, 'pred_res2.shape:', pred_res2.shape, 'real_res.shape:', real_res.shape)
  res = np.concatenate((real_res, pred_res1, pred_res2), axis=1)
  print ('real-label\t pred-label\tpred-probability\n', res)
