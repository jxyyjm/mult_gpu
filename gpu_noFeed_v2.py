...
'''
	非feed流下，训练与预测；
	多GPU-Test.
	加入滑动平均 tf.train.ExponentialMovingAverage
'''
import os
import sys
reload(sys)
...
def main():	
	...
	if predict_flag == False:
		...
		net_used     = net_model(model_dir)
		ema          = tf.train.ExponentialMovingAverage(decay=0.999)
		#restore_map  = {ema.average_name(net_used.w1): net_used.w1, ema.average_name(net_used.w2: net_used.w2)}
		## 若是想仅仅恢复部分的Move_avg 变量，可以用ema.average_name(model.var) 来获取变量对应的滑动变量名称 ##
