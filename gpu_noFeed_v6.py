...
model_dir    = './model_v6'
dataset_flow = get_batch_data(train_data_file, buffer_size=1000, batch_size=16, epoch=1000, shuffle=True)
x, y         = dataset_flow
...
#	c_w_1  = tf.get_variable(name='w1', initializer=tf.random_normal((column_num, 1), mean=0.0, stddev=0.002), dtype=tf.float32, regularizer= regularizer)
...
for i in xrange(4):
	with tf.device('/gpu:' +str(i)):
		with tf.name_scope('name_scope-'+str(i)) as scope:
			...
			tf.losses.sigmoid_cross_entropy(multi_class_labels = y, logits = tf.squeeze(logits, -1))
			...
			with tf.control_dependencies(update_ops): 
				...
				gradients, _ = tf.clip_by_global_norm(t_list=gradients, clip_norm=10.0) ## clip ##
