...
'''
	...
	在测试时，试验是否可以通过加载图的方式，避免想v4.1里面的重新将图定义一遍
'''
import os
import sys
reload(sys)
...
class net_model:
	...
	def logits(self, x, training=True):
	...
	def infernce(self, x, y=None):
		logits     = self.logits(x, training=False)
		...
		if y is not None:
			...
			prob_class = tf.cast(tf.expand_dims(prob_class, 1), dtype=tf.float32)
			pred_class = tf.cast(tf.expand_dims(pred_class, 1), dtype=tf.float32)	
			return tf.concat([prob_class, pred_class], 1)
	def eval(self, x, y):
			...
			for i in xrange(4):
				with tf.device('/gpu:' +str(i)):
					with tf.name_scope('name_scope-'+str(i)) as scope:
						logits = self.logits(x_list[i], training=True)
	...
	if predict_flag == False:
		...
		if ckpt_file:
			with tf.Session() as sess:
				saver = tf.train.import_meta_graph('./model_v4/ckpt-30001.meta')
				saver.restore(sess, ckpt_file)
				sess.run(tf.tables_initializer())
				print 'tf.global_variables', tf.global_variables()
				print 'graph.operations', tf.Graph.get_operations()
				
#				下面这里检查，bn/moving_mean是否恢复正常 ## 答案是正常 ##
#				from tensorflow.python import pywrap_tensorflow
#				reader = pywrap_tensorflow.NewCheckpointReader(ckpt_file)
#				print 'layer-1/bn/moving_mean', reader.get_tensor('layer-1/batch_normalization/moving_mean')
#				print 'layer-1/bn/moving_variance', reader.get_tensor('layer-1/batch_normalization/moving_variance')
#				print 'layer-2/bn/moving_mean', reader.get_tensor('layer-2/batch_normalization/moving_mean')
#				print 'layer-2/bn/moving_variance', reader.get_tensor('layer-2/batch_normalization/moving_variance')
