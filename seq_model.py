...
user2tags = pd.read_csv('../data/key2id.user.tags.ID.only', sep='\t', names=col_names, header=None, index_col=0, dtype=np.int32)
maxID_tag1= user2tags.max().max() # for userTagsID == > tagsHidden #
col_names = ['url_tag_'+str(i) for i in range(7)]
urls2tags = pd.read_csv('../data/key2id.url.tags.ID.only', sep='\t', names=col_names, header=None, index_col=0, dtype=np.int32)
...
with tf.device('/gpu:1'):
  ...
  with tf.variable_scope('compute'):
    ...
    print 'sim.get_shape().as_list():', type(sim.get_shape().as_list()), len(sim.get_shape().as_list())
